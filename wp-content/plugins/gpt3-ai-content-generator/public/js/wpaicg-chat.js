...
function wpaicgChatInit() {
    ...
    function wpaicgFormatter(inputText) {
        ...
        // Replace headers
        inputText = inputText.replace(/^# (.*?)$/gm, '<h1>$1</h1>');
        inputText = inputText.replace(/^## (.*?)$/gm, '<h2>$1</h2>');
        inputText = inputText.replace(/^### (.*?)$/gm, '<h3>$1</h3>');
        inputText = inputText.replace(/^#### (.*?)$/gm, '<h4>$1</h4>');
        inputText = inputText.replace(/^##### (.*?)$/gm, '<h5>$1</h5>');
        inputText = inputText.replace(/^###### (.*?)$/gm, '<h6>$1</h6>');

        // Replace new lines
        inputText = inputText.replace(/(?:\r\n|\r|\n)/g, '<br>');
    
        // Replace blockquotes
        inputText = inputText.replace(/^>\s*(.*)$/gm, '<blockquote>$1</blockquote>');
    
        // Replace unordered lists
        inputText = inputText.replace(/^\s*-\s+(.*)$/gm, '<ul><li>$1</li></ul>');
        
        // Replace ordered lists
        inputText = inputText.replace(/^\s*\d+\.\s+(.*)$/gm, '<ol><li>$1</li></ol>');
    
        // Replace horizontal rules
        inputText = inputText.replace(/^\s*---\s*$/gm, '<hr>');
        ...
        // Replace code blocks
        inputText = inputText.replace(/```([\s\S]*?)```/g, '<pre><code>$1</code></pre>');
    ...
    function handleStreaming(wpaicgData, wpaicg_messages_box, wpaicg_box_typing, wpaicg_ai_thinking, class_ai_item, chat, chatbot_identity, clientID, wpaicg_use_avatar, wpaicg_ai_avatar) {
        ...
        function processBuffer() {
            ...
            // Replace bold text
            let updatedBuffer = buffer.replace(/\*\*(.*?)\*\*/g, '<b>$1</b>');
            
            // Replace headers
            updatedBuffer = updatedBuffer.replace(/^# (.*?)$/gm, '<h1>$1</h1>');
            updatedBuffer = updatedBuffer.replace(/^## (.*?)$/gm, '<h2>$1</h2>');
            updatedBuffer = updatedBuffer.replace(/^### (.*?)$/gm, '<h3>$1</h3>');
            updatedBuffer = updatedBuffer.replace(/^#### (.*?)$/gm, '<h4>$1</h4>');
            updatedBuffer = updatedBuffer.replace(/^##### (.*?)$/gm, '<h5>$1</h5>');
            updatedBuffer = updatedBuffer.replace(/^###### (.*?)$/gm, '<h6>$1</h6>');

            // Replace new lines
            updatedBuffer = updatedBuffer.replace(/(?:\r\n|\r|\n)/g, '<br>');

            // Replace blockquotes
            updatedBuffer = updatedBuffer.replace(/^>\s*(.*)$/gm, '<blockquote>$1</blockquote>');

            // Replace unordered lists
            updatedBuffer = updatedBuffer.replace(/^\s*-\s+(.*)$/gm, '<ul><li>$1</li></ul>');
            
            // Replace ordered lists
            updatedBuffer = updatedBuffer.replace(/^\s*\d+\.\s+(.*)$/gm, '<ol><li>$1</li></ol>');

            // Replace horizontal rules
            updatedBuffer = updatedBuffer.replace(/^\s*---\s*$/gm, '<hr>');
            
            // Replace italic text
            updatedBuffer = updatedBuffer.replace(/\*(.*?)\*/g, '<i>$1</i>');
            
            // Replace underline text
            updatedBuffer = updatedBuffer.replace(/__(.*?)__/g, '<u>$1</u>');
            
            // Replace strikethrough text
            updatedBuffer = updatedBuffer.replace(/~~(.*?)~~/g, '<s>$1</s>');

            // replace markdown links
            updatedBuffer = updatedBuffer.replace(/\[(.*?)\]\((https?:\/\/.*?)\)/g, '<a href="$2" target="_blank">$1</a>');
            
            // Replace code blocks
            updatedBuffer = updatedBuffer.replace(/```([\s\S]*?)```/g, '<pre><code>$1</code></pre>');
            
            // Replace inline code
            updatedBuffer = updatedBuffer.replace(/`([\s\S]*?)`/g, '<code>$1</code>');
            
            document.getElementById(chatId).innerHTML = updatedBuffer;
        }
    
        function updateChatHistory(message, sender) {
            const key = `wpaicg_chat_history_${chatbot_identity}_${clientID}`;
            const history = JSON.parse(localStorage.getItem(key) || '[]');
            history.push(`${sender === 'user' ? "Human: " : "AI: "} ${message.trim()}`);
